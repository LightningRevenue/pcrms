  ...
  return (
    <div className="px-8 py-10 max-w-2xl">
      ...
      <div className="flex items-center justify-between mt-10">
        <div>
          <h2 className="text-[15px] font-medium">Custom dashboards</h2>
          <p className="text-[12px] text-subtle mt-0.5">Combine several reports into one page.</p>
        </div>
        <Link
          href="/dashboards/custom/new"
      ...
      <div className="mt-3 border border-border rounded-md overflow-hidden">
        {customDashboards.length === 0 ? (
          <div className="px-4 py-6 text-center text-[13px] text-subtle">No custom dashboards yet.</div>
        ) : (
          customDashboards.map((d) => (
            <Link
              key={d.id}
              href={`/dashboards/custom/${d.id}`}
              className="flex items-center gap-3 px-4 py-3 hover:bg-muted/40 transition-colors border-b border-border last:border-b-0"
            >
              <LayoutDashboard size={16} strokeWidth={1.5} className="text-subtle shrink-0" />
              <span className="text-[13px]">{d.name}</span>
            </Link>
          ))
        )}
